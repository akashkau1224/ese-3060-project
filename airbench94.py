...
def should_skip_agc(name, param):
    return 'bias' in name.lower() or 'norm' in name.lower() or param.ndim <= 1
...
def adaptive_gradient_clipping(model, clip_factor=0.01, eps=1e-6):
    for name, param in model.named_parameters():
        if param.grad is None or should_skip_agc(name, param):
            continue
        p_norm = unitwise_norm(param.data)
        g_norm = unitwise_norm(param.grad.data)
        numerator = clip_factor * (p_norm + eps)
        clipped_grad = param.grad.data * (numerator / (g_norm + eps))
